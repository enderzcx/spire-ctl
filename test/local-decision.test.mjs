...
const combat=(overrides={})=>({state_type:'monster',run:{act:1,floor:4,seed:'run-local'},player:{hp:40,max_hp:80,block:0,energy:3,
...
test('the original cutoff still escalates when confidence is low',async()=>{
...
test('an unanswerable fight stops even while a strategy would carry it',()=>temporary(async dir=>{
  ...
  await saveStrategy(dir,{strategy_id:'carry',created_floor:4,reason:'chip',
    run_identity:JSON.stringify({id:'run-local',character:null}),
