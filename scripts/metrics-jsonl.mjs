...
function parseArgs(argv){
  ...
  for(let index=1;index<argv.length;index++){
    ...
    if(flag==='--compact')options.compact=true;
      ...
      options.protocol=value==='all'?null:Number(value);
    }
...
function table(report,limit){
  ...
  for(const batch of report.batches){
    ...
    const shownTurns=batch.turns.slice(-limit);
    ...
    if(batch.turns.length>shownTurns.length)lines.push(`  (${batch.turns.length-shownTurns.length} earlier turns omitted)`);
  }
...
if(available.length>1&&options.protocol===null)
  console.log('warning: this file mixes decision protocols; pass --protocol N (or all) for a single-window report');
if(options.compact){
